...
class Receptores extends Component{
   ...
   async deleteItem(id){
        ...
        await ax.delete(`http://localhost:8081/item/produtoDelReceptor/${id}`).then(() => {
    ...
    getProduto(){
        ax.get('http://localhost:8081/item/receptor').then(response => 
    ...
    render(){
        ...
        return(
            <div>
                <NavHsc/>
                <Nav/>
                <h1 id="title_h1_receptores">Receptores</h1>
                    ...
                    items.map(produtos => {
                        ...
                        return(
                                <div key={id} id="container_product">
                                    <button id="btn_delete_receptor" onClick={() => this.deleteItem(id)}>X</button>
                                    <img id="img_receptor" src={`/image/receptores/${img}`} alt="img_produto"/>
                                    ...
                                    <button id="btn_acessar"><Link to={`/produtoReceptor/item/${id}`} id="link_btn" >Acessar</Link></button>         
                                </div>
                        );
                    })
                ) 
