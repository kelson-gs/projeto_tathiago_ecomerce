...
class Cabos extends Component{
   ...
   async deleteItem(id){
        ...
        await ax.delete(`http://localhost:8081/item/produtoDelCabo/${id}`).then(() => {
    ...
    getProduto(){
        ax.get('http://localhost:8081/item/cabo').then(response => 
                    ...
                    items.map(produtos => {
                        ...
                        return(
                                <div key={id} id="container_product">
                                    <button id="btn_delete_cabo" onClick={() => this.deleteItem(id)}>X</button>
                                    <img id="img_cabo" src={`/image/cabos/${img}`} alt="img_produto"/>
                                    ...
                                    <button id="btn_acessar"><Link to={`/produtoCabo/item/${id}`} id="link_btn" >Acessar</Link></button>         
                                </div>
                        );
                    })
                ) 
