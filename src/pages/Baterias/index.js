...
class Baterias extends Component{
   ...
   async deleteItem(id){
        ...
        await ax.delete(`http://localhost:8081/item/produtoDelBateria/${id}`).then(() => {
    ...
    getProduto(){
        ax.get('http://localhost:8081/item/bateria').then(response => 
    ...
    render(){
        ...
        return(
            <div>
                <NavHsc/>
                <Nav/>
                <h1 id="title_h1_baterias">Baterias</h1>
                    ...
                    items.map(produtos => {
                        ...
                        return(
                                <div key={id} id="container_product">
                                    <button id="btn_delete_bateria" onClick={() => this.deleteItem(id)}>X</button>
                                    <img id="img_bateria" src={`/image/baterias/${img}`} alt="img_produto"/>
                                    ...
                                    <button id="btn_acessar"><Link to={`/produtoBateria/item/${id}`} id="link_btn" >Acessar</Link></button>         
                                </div>
                        );
                    })
                ) 
