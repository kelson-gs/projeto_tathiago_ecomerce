...
class Roteadores extends Component{
   ...
   async deleteItem(id){
        ...
        await ax.delete(`http://localhost:8081/item/produtoDelRoteador/${id}`).then(() => {
    ...
    getProduto(){
        ax.get('http://localhost:8081/item/roteador').then(response => 
    ...
    render(){
        ...
        return(
            <div>
                <NavHsc/>
                <Nav/>
                <h1 id="title_h1_roteadores">Roteadores</h1>
                    ...
                    items.map(produtos => {
                        ...
                        return(
                                <div key={id} id="container_product">
                                    <button id="btn_delete_roteador" onClick={() => this.deleteItem(id)}>X</button>
                                    <img id="img_roteador" src={`/image/roteadores/${img}`} alt="img_produto"/>
                                    ...
                                    <button id="btn_acessar"><Link to={`/produtoRoteador/item/${id}`} id="link_btn" >Acessar</Link></button>         
                                </div>
                        );
                    })
                ) 
