...
class Controles extends Component{
   ...
   async deleteItem(id){
        ...
        await ax.delete(`http://localhost:8081/item/produtoDelControle/${id}`).then(() => {
    ...
    getProduto(){
        ax.get('http://localhost:8081/item/controle').then(response => 
    ...
    render(){
        ...
        return(
            <div>
                <NavHsc/>
                <Nav/>
                <h1 id="title_h1_controles">Controles</h1>
                    ...
                    items.map(produtos => {
                        ...
                        return(
                                <div key={id} id="container_product">
                                    <button id="btn_delete_controle" onClick={() => this.deleteItem(id)}>X</button>
                                    <img id="img_controle" src={`/image/controles/${img}`} alt="img_produto"/>
                                    ...
                                    <button id="btn_acessar"><Link to={`/produtoControle/item/${id}`} id="link_btn" >Acessar</Link></button>         
                                </div>
                        );
                    })
                ) 
