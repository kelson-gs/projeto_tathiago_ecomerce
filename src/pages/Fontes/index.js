...
class Fontes extends Component{
   ...
   async deleteItem(id){
        ...
        await ax.delete(`http://localhost:8081/item/produtoDelFonte/${id}`).then(() => {
    ...
    getProduto(){
        ax.get('http://localhost:8081/item/fonte').then(response => 
    ...
    render(){
        ...
        return(
            <div>
                <NavHsc/>
                <Nav/>
                <h1 id="title_h1_fontes">Fontes</h1>
                    ...
                    items.map(produtos => {
                        ...
                        return(
                                <div key={id} id="container_product">
                                    <button id="btn_delete_fonte" onClick={() => this.deleteItem(id)}>X</button>
                                    <img id="img_fonte" src={`/image/fontes/${img}`} alt="img_produto"/>
                                    ...
                                    <button id="btn_acessar"><Link to={`/produtoFonte/item/${id}`} id="link_btn" >Acessar</Link></button>         
                                </div>
                        );
                    })
                ) 
