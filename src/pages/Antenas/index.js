...
class Antenas extends Component{
   ...
   async deleteItem(id){
        ...
        await ax.delete(`http://localhost:8081/item/produtoDelAntena/${id}`).then(() => {
    ...
    getProduto(){
        ax.get('http://localhost:8081/item/antena').then(response => 
    ...
    render(){
        ...
        return(
            <div>
                <NavHsc/>
                <Nav/>
                <h1 id="title_h1_antenas">Antenas</h1>
                    ...
                    items.map(produtos => {
                        ...
                        return(
                                <div key={id} id="container_product">
                                    <button id="btn_delete_antena" onClick={() => this.deleteItem(id)}>X</button>
                                    <img id="img_antena" src={`/image/antenas/${img}`} alt="img_produto"/>
                                    ...
                                    <button id="btn_acessar"><Link to={`/produtoAntena/item/${id}`} id="link_btn" >Acessar</Link></button>         
                                </div>
                        );
                    })
                ) 
